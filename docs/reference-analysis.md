...
- Desktop runtime: Windows uses the same TDLib workflow, but TDJSON is resolved from a valid configured DLL path or from `tdjson.dll` beside the executable. The generated Windows runner automatically bundles API-compatible TDLib 1.8.21.2 plus app-local Visual C++ dependencies for Debug, Release, and packaged builds, while a version marker forces incompatible native DLLs to be replaced and a repository secret can still inject a custom DLL. Initialization uses a minimal stable JSON request instead of the binding's legacy generated fields. If the native TDLib client restarts, authorized requests reapply initialization parameters and retry once before surfacing an error.
