...
- Media discovery: the reference scans configured Telegram channels, reads forwarded messages, identifies media by message content, and relies heavily on filenames/captions for title, quality, split files, and episode metadata.
...
- Partial download behavior: the FastAPI streamer fetches chunks from Telegram through a `ByteStreamer` and prefetches adjacent chunks. This Flutter app applies the same essential pattern through a localhost HTTP server consumed by `video_player`.
- Large files: the reference never requires loading an entire file into memory. It streams 1 MB-style ranges and tracks active streams. This app also serves chunked ranges from TDLib downloads.
- Split files: the reference detects trailing `.ext.001`, `.ext.002` style parts and exposes them as one virtual stream. This app keeps the same split detection and virtual byte offset model.
